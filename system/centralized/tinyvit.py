"""
Centralized training script for ISIC2019 using TinyViT
...
class ISIC2019CentralizedDataset(Dataset):
    """Combined ISIC2019 dataset from all federated clients"""
    
    def __init__(self, data_dir='ISIC2019', train=True, transform=None):
        """
        Args:
            data_dir: Path to ISIC2019 dataset directory
...
def get_transforms():
    """Define training and testing augmentations"""
    ...
    # Post-processing function to normalize
    ...
    return train_transform, test_transform, normalize_image
...
def plot_confusion_matrix(y_true, y_pred, num_classes, save_path='confusion_matrix_tinyvit.png'):
    ...
    plt.title('Confusion Matrix (TinyViT)', fontsize=16, fontweight='bold')
...
def plot_roc_curves(y_true, y_probs, num_classes, save_path='roc_curves_tinyvit.png'):
    ...
    ax.set_title('ROC Curves (One-vs-Rest, TinyViT)', fontsize=16, fontweight='bold')
...
def plot_pr_curves(y_true, y_probs, num_classes, save_path='pr_curves_tinyvit.png'):
    ...
    ax.set_title('Precision-Recall Curves (TinyViT)', fontsize=16, fontweight='bold')
...
def plot_overall_roc_curve(y_true, y_probs, num_classes, save_path='overall_roc_curve_tinyvit.png'):
    ...
    ax.set_xlabel('False Positive Rate', fontsize=12)
    ax.set_ylabel('True Positive Rate', fontsize=12)
    ax.set_title('Overall ROC Curve (TinyViT)', fontsize=14, fontweight='bold')
    ax.grid(alpha=0.3)
    ax.legend(loc='lower right', fontsize=10)
...
def plot_overall_pr_curve(y_true, y_probs, num_classes, save_path='overall_pr_curve_tinyvit.png'):
    ...
    ax.set_xlabel('Recall', fontsize=12)
    ax.set_ylabel('Precision', fontsize=12)
    ax.set_title('Overall Precision-Recall Curve (TinyViT)', fontsize=14, fontweight='bold')
    ax.grid(alpha=0.3)
...
def main():
    ...
    output_dir = os.path.join(script_dir, 'tinyvit_outputs')
    os.makedirs(output_dir, exist_ok=True)
    print(f"Output directory: {output_dir}")
    
    # Load dataset
    print("\n=== Loading ISIC2019 Dataset ===")
    ...
    data_dir = '../../dataset/ISIC2019'
    
    # Load all training data
    full_dataset = ISIC2019CentralizedDataset(
        data_dir=data_dir, 
        train=True,
    ...
    # Load test data
    test_dataset = ISIC2019CentralizedDataset(
        data_dir=data_dir,
        train=False,
    ...
    # Create model - TinyViT
    print("\n=== Creating TinyViT Model ===")
    try:
        import timm
    except Exception:
    ...
    # Wrapper class to properly handle TinyViT forward pass
    ...
    model = TinyVitClassifier(model, NUM_CLASSES)
    ...
    best_val_acc = 0
    best_epoch = 0
    best_model_path = os.path.join(output_dir, 'best_tinyvit_isic2019.pth')
    ...
    print("\n=== Generating Plots ===")
    confusion_matrix_path = os.path.join(output_dir, 'confusion_matrix_tinyvit.png')
    roc_curves_path = os.path.join(output_dir, 'roc_curves_per_class_tinyvit.png')
    pr_curves_path = os.path.join(output_dir, 'pr_curves_per_class_tinyvit.png')
    overall_roc_path = os.path.join(output_dir, 'overall_roc_curve_tinyvit.png')
    overall_pr_path = os.path.join(output_dir, 'overall_pr_curve_tinyvit.png')
    ...
    print("\n=== Summary ===")
    print(f"Centralized TinyViT on ISIC2019:")
