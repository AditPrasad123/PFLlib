...
def parse_args():
    parser = argparse.ArgumentParser(
        description="True class-specific Grad-CAM for centralized QuanvEfficientNetB0 on ISIC2019_quanv."
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default=os.path.join(SCRIPT_DIR, "quanv_efficientnetb0_outputs", "gradcam_outputs"),
    )
...
def collect_predictions(model, dataset, batch_size, device):
    ...
    # Load all client test splits for centralized analysis.
    client_id = 0
    while True:
        try:
...
def main():
    ...
    # Enable gradients for Grad-CAM regardless of any frozen params.
    ...
    print("\nCentralized Grad-CAM analysis complete.")
