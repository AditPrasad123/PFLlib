...
def parse_args():
    parser = argparse.ArgumentParser(
        description="False-negative analysis and t-SNE for FedBABU personalized client checkpoints."
    )
...
def default_paths(dataset, run_id):
    ...
    output_dir = os.path.join(
        ...
        "..",
        "results",
        f"analysis_{dataset}_FedBABU_personalized_run_{run_id}",
    )
...
def collect_for_client(model, client_id, dataset, batch_size, device):
    model.eval()

    head = get_head_module(model)
    if head is None:
        raise RuntimeError(f"Could not find model head for client {client_id}.")
    ...
    with torch.no_grad():
        ...
        for x, y in loader:
            ...
            if captured["emb"] is None:
                raise RuntimeError(f"Failed to capture embeddings for client {client_id}.")
...
def save_tsne_plots(emb_2d, labels, preds, output_dir):
    ...
    plt.title("t-SNE of personalized-test embeddings (color = true class)")
    ...
    plt.legend(loc="best")
    plt.title("t-SNE with false negatives highlighted (personalized clients)")
    plt.xlabel("t-SNE dim 1")
    plt.ylabel("t-SNE dim 2")
    plt.tight_layout()
...
def main():
    ...
    ckpts = sorted(glob.glob(os.path.join(clients_dir, "client_*.pt")))
    if not ckpts:
        raise FileNotFoundError(f"No client checkpoints found in: {clients_dir}")
    ...
    for ckpt in ckpts:
        client_id = int(os.path.splitext(os.path.basename(ckpt))[0].split("_")[-1])
        print(f"Processing personalized client {client_id}: {ckpt}")

        model = torch.load(ckpt, map_location=device, weights_only=False)
        model.to(device)

        records, emb, labels, preds = collect_for_client(
            model=model,
            client_id=client_id,
    ...
    save_prediction_csvs(all_records, output_dir)
    save_fn_summary(build_fn_summary(all_records), output_dir)
    ...
    print("\nPersonalized FN + t-SNE analysis complete.")
